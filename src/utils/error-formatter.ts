...
export interface HopErrorOptions {
  step?: Step;
  context?: TestContext;
  featureName?: string;
  scenarioName?: string;
...
export class HopError extends Error {
  ...
  constructor(message: string, options?: HopErrorOptions) {
    super(message);
    this.name = 'HopError';
  ...
  getDetailedMessage(): string {
    ...
    if (this.featureName || this.scenarioName) {
      output += `\n📍 Location:\n`;
      if (this.featureName) output += `   Feature: ${this.featureName}\n`;
      if (this.scenarioName) output += `   Scenario: ${this.scenarioName}\n`;
      if (this.featurePath) output += `   File: ${this.featurePath}\n`;
    }
...
export function formatValue(value: any, maxLength = 200): string {
...
export function generateDiff(expected: any, actual: any): string | null {
...
export function generateStepSuggestion(stepText: string): string[] {
  const suggestions: string[] = [];
  const lowerText = stepText.toLowerCase();

  if (lowerText.includes('navigates')) {
    suggestions.push(
      'Make sure you have "Given user opens browser" before navigating',
  ...
  if (lowerText.includes('clicks')) {
    suggestions.push(
      ...
      'Try adding wait step: "And user waits for \'selector\' to be clickable"',
  ...
  if (lowerText.includes('types')) {
    suggestions.push(
      'Make sure the input field is visible and enabled',
      'Check if the selector targets the correct input element'
    );
  }

  if (lowerText.includes('status')) {
    suggestions.push(
      'Check the API endpoint URL and method',
  ...
  if (lowerText.includes('match response')) {
    suggestions.push(
      'Verify the response structure matches your assertion',
      'Use "And match response != null" to check if response exists first',
      'Check for typos in the JSON path (e.g., response.data.user.name)'
    );
  }
...
export function createErrorFromStep(
  step: Step,
  error: Error,
  context?: TestContext
): HopError {
...
export function isUndefinedStepError(error: any): boolean {
