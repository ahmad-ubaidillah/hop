...
export class OpenAPIImporter {
  ...
  private options: OpenAPIImportOptions;

  constructor(spec: any, options: OpenAPIImportOptions = {}) {
    this.spec = spec;
   ...
   * Import from URL
   */
  static async fromUrl(url: string, options?: OpenAPIImportOptions): Promise<OpenAPIImporter> {
   ...
   * Import from file
   */
  static fromFile(spec: any, options?: OpenAPIImportOptions): OpenAPIImporter {
    return new OpenAPIImporter(spec, options);
  }
   ...
   */
  filterByTags(tags: string[]): OpenAPIEndpoint[] {
   ...
   */
  generateFeature(endpoints?: OpenAPIEndpoint[]): string {
   ...
   */
  private generateScenariosForEndpoint(endpoint: OpenAPIEndpoint): string[] {
    const lines: string[] = [];
   ...
   */
  private groupByTags(endpoints: OpenAPIEndpoint[]): Record<string, OpenAPIEndpoint[]> {
   ...
   * Format feature name
   */
  private formatFeatureName(tag: string): string {
    return tag
      .replace(/-/g, ' ')
   ...
   */
  private formatScenarioName(endpoint: OpenAPIEndpoint, type: string, errorCode?: string): string {
   ...
   */
  private generateExample(schema: any, name: string): any {
   ...
   */
  private generateRequestBodyExample(requestBody: OpenAPIRequestBody): any {
    const contentTypes = Object.keys(requestBody.content);
    if (contentTypes.length === 0) return {};
    
    const contentType = contentTypes[0] as string;
    ...
    if (mediaType.examples) {
      ...
      if (examples.length > 0) {
        const firstExample = examples[0] as { value?: any };
