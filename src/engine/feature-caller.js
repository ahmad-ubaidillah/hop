...
export class FeatureCaller {
    ...
    async loadFeature(featurePath) {
        ...
        try {
            const content = await import('fs/promises').then(fs => fs.readFile(featurePath, 'utf-8'));
 ...
 */
let globalFeatureCaller = null;
/**
 * Get or create the global feature caller
 */
export function getFeatureCaller(basePath = './features') {
 ...
 */
export function resetFeatureCaller() {
